...
export default class PrivateArchiList extends Component {
    ...
    constructor(props) {
        super(props);
        this.state = ({ localArreglo: JSON.parse(localStorage.getItem('listArchi')) ? Array.from(JSON.parse(localStorage.getItem('listArchi'))) : [] })
    }
    ...
    deleteBuilding(id) {
        let jsonArreglo = [];
        this.setState({ localArreglo: this.state.localArreglo.filter((element) => element.id !== id).map((element, index) => { element.id = index; return element }) },
            () => {
